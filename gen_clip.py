"""Generate llm_basic/05_clip.ipynb — CLIP multimodal embeddings"""
...
cells.append(code(code_clip))

cells.append(md(
    "## 6. Interview Q&A\n\n"
    "**Q1: How does CLIP create 1 billion negatives per training step?**\n"
    "> With a batch size of N=32,768, CLIP computes an N×N similarity matrix. "
    "There are N correct (positive) pairs on the diagonal. "
    "All other N(N-1) ≈ 1 billion off-diagonal entries are implicit negatives — "
    "each image paired with every non-matching text, and vice versa. "
    "This massive negative set is what makes contrastive learning so data-efficient "
    "compared to supervised methods.\n\n"
    "---\n\n"
    "**Q2: What is the role of temperature τ in InfoNCE loss?**\n"
    "> Temperature τ scales the logits before softmax. Small τ → sharp distribution (model can be "
    "very confident, but gradients may vanish for 'almost correct' pairs). Large τ → soft distribution "
    "(more training signal from hard negatives, but learning is slower). "
    "CLIP treats τ as a learnable parameter, initialised to 0.07, and the model learns the "
    "optimal sharpness. Empirically, a temperature around 0.02–0.07 works best.\n\n"
    "---\n\n"
    "**Q3: Why does prompt engineering matter for CLIP zero-shot?**\n"
    "> CLIP was trained on web captions like 'a photo of a cat playing in snow' — not just 'cat'. "
    "When doing zero-shot classification with just the word 'cat', the embedding doesn't match "
    "the training distribution well. Wrapping labels in templates like 'a photo of {class}' or "
    "'a photo of a {class} in the wild' bridges this distribution gap and improves accuracy "
    "by 3–5% on ImageNet. Ensembling multiple prompt variants improves further.\n\n"
    "---\n\n"
    "**Q4: What does ALIGN show that was surprising?**\n"
    "> ALIGN showed that training on 1.8 billion NOISY image-text pairs (no filtering) "
    "outperforms CLIP's 400M carefully filtered pairs. This suggests that at sufficient scale, "
    "the model learns to average out noise and still extract meaningful signal. "
    "This challenged the belief that data quality is always more important than quantity — "
    "at scale, quantity wins. This principle later influenced LLM training (Common Crawl = noisy but huge).\n\n"
    "---\n\n"
    "**Q5: What are the most important applications of CLIP-like models?**\n"
    "> (1) **Zero-shot image classification** — classify any image without retraining. "
    "(2) **Image-text retrieval** — given a text query, retrieve matching images from a database. "
    "(3) **Text-to-image generation** — DALL-E 2/Stable Diffusion use CLIP text embeddings as conditioning. "
    "(4) **Image captioning** — generate text descriptions from image embeddings. "
    "(5) **Video understanding** — extend CLIP to video frames for action recognition. "
    "(6) **Medical AI** — BioViL extends CLIP to radiology reports + chest X-rays."
))

nb.cells = cells
nbf.write(nb, 'llm_basic/05_clip.ipynb')
print(f"Written: llm_basic/05_clip.ipynb  ({len(cells)} cells)")
